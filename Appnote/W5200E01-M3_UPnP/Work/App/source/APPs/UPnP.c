...
#define MAX_BUFFER		(1024*4)	/**< Send & Receive Buffer Size */
...
char descURL[64];		/**< Description URL */
char descIP[16];		/**< Description IP */
char descPORT[6];		/**< Description Port */
char descLOCATION[64];		/**< Description Location */
char controlURL[64];		/**< Control URL */
char eventSubURL[64];		/**< Eventing Subscription URL */

char content[MAX_BUFFER]={'\0'};		/**< HTTP Body */
char send_buffer[MAX_BUFFER]={'\0'};		/**< Send Buffer */
char recv_buffer[MAX_BUFFER]={'\0'};		/**< Receive Buffer */
...
//	setSn_DPORT(sockfd,mcast_mac);
	...
	endTime = my_time + 3;
	while (UDPRecv(sockfd, (uint8*)recv_buffer, MAX_BUFFER, recv_addr, &recv_port) <= 0 && my_time < endTime);	// Check Receive Buffer of W5200
