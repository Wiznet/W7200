...
uint8 IP[4] = {192, 168, 11, 5};				//IP Address
...
uint8 FTP_SERVER_IP[4] = {192, 168, 11, 202};			//FTP Server IP Address

uint8 TX_BUF[TX_RX_MAX_BUF_SIZE];
uint8 RX_BUF[TX_RX_MAX_BUF_SIZE];

uint8 FTP_CLIENT_PI_SOCK = 0;
uint8 FTP_CLIENT_DTP_SOCK = 1;
...
void GPIO_Configuration(void)
{
    ...
    // Port B Interrupt
    GPIO_InitStructure.GPIO_Pin = WIZ_INT;//PB0
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
